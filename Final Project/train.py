...
def trainer(args):
    ...
    # CSV file to store PSNR values
    psnr_csv_path = os.path.join(checkpoint_dir, 'psnr_log.csv')
    with open(psnr_csv_path, mode='w') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(['Step', 'Validation PSNR', 'Highest PSNR'])
    ...
    ds_image1, ds_image2 = pair_downsampler_tf(image_train)
    ...
    out_train_unmasked_1 = CBSN(ds_image1, filters, N, False, reuse=True)
    out_train_unmasked_2 = CBSN(ds_image2, filters, N, False, reuse=True)
    ...
    loss_unmasked_self = (tf.reduce_mean(tf.abs(out_train_unmasked_1 - ds_image2))+tf.reduce_mean(tf.abs(out_train_unmasked_2 - ds_image1)))/2
    ...
    with tf.Session() as sess:
        ...
        for i in range(start, max_iter):
            ...
            if i % args.val_every == 0:
                ...
                # Update highest PSNR if current one is higher
                if psnr_val > highest_psnr:
                    highest_psnr = psnr_val
                
                # Write to CSV
                with open(psnr_csv_path, mode='a') as csv_file:
                    writer = csv.writer(csv_file)
                    writer.writerow([i, psnr_val, highest_psnr])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--print_every", type=int, default=500, help='Print interval')
    parser.add_argument("--val_every", type=int, default=1000, help='Validation interval')
    parser.add_argument("--save_every", type=int, default=5000, help='Save interval')
    ...
    parser.add_argument("--name", type=str, default='temp6', help='Name of experiments')
