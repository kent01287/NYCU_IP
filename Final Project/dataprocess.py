...
def pair_downsampler_tf(img):
    channels = img.shape[-1]  # Equivalent to the number of channels in PyTorch (C)

    # Define the first filter: [[0, 0.5], [0.5, 0]]
    filter1 = tf.constant([[[0.0], [0.5]], [[0.5], [0.0]]], dtype=tf.float32)
    filter1 = tf.expand_dims(filter1, axis=-1)  # Now filter1 shape is [2, 2, 1, 1]
    filter1 = tf.tile(filter1, [1, 1, channels, 1])  # Repeat across channels

    # Define the second filter: [[0.5, 0], [0, 0.5]]
    filter2 = tf.constant([[[0.5], [0.0]], [[0.0], [0.5]]], dtype=tf.float32)
    filter2 = tf.expand_dims(filter2, axis=-1)  # Now filter2 shape is [2, 2, 1, 1]
    filter2 = tf.tile(filter2, [1, 1, channels, 1])  # Repeat across channels

    # Apply the first filter using tf.nn.depthwise_conv2d
    output1 = tf.nn.depthwise_conv2d(img, filter1, strides=[1, 2, 2, 1], padding='VALID')

    # Apply the second filter using tf.nn.depthwise_conv2d
    output2 = tf.nn.depthwise_conv2d(img, filter2, strides=[1, 2, 2, 1], padding='VALID')
